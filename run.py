...
class TaskApp:
    ...
    def load_icons(self):
        ...
        def svg_to_imgtk(svg_file, size=(24,24)):
            try:
                ...
                img = ImageTk.PhotoImage(Image.open(io.BytesIO(png_data)).resize(size, Image.LANCZOS))
                ...
                if os.path.exists(png_file):
                    ...
                    self._icon_refs.append(img)
                    return img
                img = ImageTk.PhotoImage(Image.new('RGBA', size, (200,200,200,0)))
                self._icon_refs.append(img)
                return img
    ...
    def setup_ui(self):
        ...
        logo_label = tk.Label(logo_frame, image=self.icons['logo'])
        logo_label.pack(side="left", padx=(10,5))
        title_label = tk.Label(logo_frame, text="Lista de Tareas", font=("Segoe UI", 20, "bold"))
        ...
        self.task_entry = tk.Entry(entry_frame, font=("Segoe UI", 14), relief="flat", bd=2, bg='#fffdfa')
        self.task_entry.grid(row=0, column=0, sticky="ew", padx=(0,5), ipady=4)
        self.add_button = tk.Button(entry_frame, image=self.icons['add'], command=self.add_task, relief="flat", bd=0, width=36, height=36, bg='#dddddd')
        ...
        self.task_listbox = tk.Listbox(list_frame, font=("Segoe UI", 14), relief="flat", selectmode=tk.SINGLE, activestyle="none", highlightthickness=1, bg='#fffdfa')
        ...
        self.delete_button = tk.Button(btn_frame, image=self.icons['delete'], command=self.delete_task, relief="flat", bd=0, width=36, height=36, bg='#dddddd')
        self.delete_button.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        ToolTip(self.delete_button, "Eliminar tarea seleccionada")
        self.complete_button = tk.Button(btn_frame, image=self.icons['complete'], command=self.mark_complete, relief="flat", bd=0, width=36, height=36, bg='#dddddd')
        ...
        # Botón de tema
        self.theme_button = tk.Button(self.root, image=self.icons['theme'], command=self.toggle_theme, relief="flat", bd=0, width=36, height=36, bg='#dddddd')
        self.theme_button.grid(row=4, column=0, columnspan=2, pady=(0,5))
        ToolTip(self.theme_button, "Cambiar tema claro/oscuro")

        # Barra de estado
        self.status_var = tk.StringVar()
        self.status_bar = tk.Label(self.root, textvariable=self.status_var, anchor="w", font=("Segoe UI", 10), fg=THEMES[self.current_theme]["fg"])
    ...
    def apply_theme(self):
        ...
        # Solo cambiar fondo de widgets principales, no de frames ni labels
        self.task_entry.config(bg="#fffdfa", fg=theme["fg"], insertbackground=theme["fg"])
        self.task_listbox.config(bg="#fffdfa", fg=theme["fg"], selectbackground=theme["highlight"], highlightbackground=theme["highlight"])
        ...
        self.status_bar.config(bg="#fffdfa", fg=theme["fg"])
